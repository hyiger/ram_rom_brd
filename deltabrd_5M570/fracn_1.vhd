--------------------------------------------------------------------------------
-- File         : fracn_1.vhd (machine generated)
-- Contains     : entity fracn_1 (architecture rtl)
-- Author       : ./fracn09.pl  (version 0.09)
-- Command Line : ./fracn09.pl -e fracn_1 100e6 1e6
-- Date         : Fri Jul 26 19:21:58 2019
...
-- Desired Output Frequency: 1000000 Hz.
-- Requested Relative Frequency Error Bounds (+/-) : 1e-07 (0.1 ppm)
--
-- Frequency Results (use_phase_accumulator = FALSE) :
--  Achieved Output Frequency: 1000000 Hz.
--  Achieved Relative Frequency Error: 0 (0 ppm)
--  Achieved Frequency Error: 0 Hz.
--
-- Frequency Results (use_phase_accumulator = TRUE) :
--  Achieved Output Frequency: 999999.977648258 Hz.
--  Achieved Relative Frequency Error: -2.23517419071868e-08 (-0.0223517419071868 ppm)
--  Achieved Frequency Error: -0.0223517419071868 Hz.
--
-- Output Jitter Parameters (use_phase_accumulator = FALSE) :
--  The fundamental frequency is 1000000 Hz.
...
--  The fundamental frequency is 0.186264514923096 Hz.
--  The amplitude is 1e-08 seconds p-p (approx).
--
-- Design Parameters (use_phase_accumulator = FALSE) :
--  Approx 10 flip flops (7 in prescaler, 1 in controller and 2 retimes).
--  The recursive controller uses approx 1 flip flop.
--  The Dual-Modulus Prescaler uses ratios /100,/101
--  The Output consists of 1 cycle of 100 input clocks,
--  and 0 cycles of 101 input clocks.
--  There is 1 output clock for every 100 input clocks.
--
-- Design Parameters (use_phase_accumulator = TRUE) :
--  Approx 31 flip flops (29 in accumulator and 2 retimes)
--  There are 5368709 output clocks for every 536870912 input clocks.
--
-- Divider summary :
--
...
--
--  31    tbd       -2.2e-08  1e-08     use_phase_accumulator
--  34    tbd       -2.2e-08  5e-09     use_phase_accumulator improve_duty_cycle
--  10    tbd       0         0         use_recursive_controller
--  11    tbd       0         0         use_recursive_controller improve_duty_cycle
--  10    tbd       0         0         minimum_jitter
--  11    tbd       0         0         minimum_jitter improve_duty_cycle
--  10    tbd       0         0         (none)
--  11    tbd       0         0         (none) improve_duty_cycle
--
-- Warnings:
--* Warning: a fractional-N divider is not needed.
--*   Integer divider 100 is ok.
--
...
entity fracn_1 is
    ...
    port (
        ...
        output_50         : out std_logic;  -- 1000000 Hz output - approx 50% duty cycle
        output_pulse      : out std_logic   -- 1000000 Hz output - high for single clock per cycle
    );
end fracn_1;

architecture rtl of fracn_1 is

    -- definitions for prescaler / controller design
    constant n              : positive := 100;  -- prescaler divides by n or n + 1
    constant a              : positive := 1;    -- this many counts of 100
    constant b              : natural  := 0;    -- this many counts of 101
    ...
    constant num_bits       : positive := 29;   -- size of phase accumulator
    constant c              : unsigned(num_bits - 1 downto 0) := to_unsigned(5368709, num_bits);
...
--------------------------------------------------------------------------------
-- Prescaler.  Divides by either 100 or 101
