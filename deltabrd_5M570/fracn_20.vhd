--------------------------------------------------------------------------------
-- File         : fracn_20.vhd (machine generated)
-- Contains     : entity fracn_20 (architecture rtl)
-- Author       : ./fracn09.pl  (version 0.09)
-- Command Line : ./fracn09.pl -e fracn_20 100e6 20e6
-- Date         : Fri Jul 26 19:21:36 2019
...
-- Desired Output Frequency: 20000000 Hz.
-- Requested Relative Frequency Error Bounds (+/-) : 1e-07 (0.1 ppm)
--
-- Frequency Results (use_phase_accumulator = FALSE) :
--  Achieved Output Frequency: 20000000 Hz.
--  Achieved Relative Frequency Error: 0 (0 ppm)
--  Achieved Frequency Error: 0 Hz.
--
-- Frequency Results (use_phase_accumulator = TRUE) :
--  Achieved Output Frequency: 19999998.8079071 Hz.
--  Achieved Relative Frequency Error: -5.96046447753906e-08 (-0.0596046447753906 ppm)
--  Achieved Frequency Error: -1.19209289550781 Hz.
--
-- Output Jitter Parameters (use_phase_accumulator = FALSE) :
--  The fundamental frequency is 20000000 Hz.
...
--  The fundamental frequency is 5.96046447753906 Hz.
--  The amplitude is 1e-08 seconds p-p (approx).
--
-- Design Parameters (use_phase_accumulator = FALSE) :
--  Approx 6 flip flops (3 in prescaler, 1 in controller and 2 retimes).
--  The recursive controller uses approx 1 flip flop.
--  The Dual-Modulus Prescaler uses ratios /5,/6
--  The Output consists of 1 cycle of 5 input clocks,
--  and 0 cycles of 6 input clocks.
--  There is 1 output clock for every 5 input clocks.
--
-- Design Parameters (use_phase_accumulator = TRUE) :
--  Approx 26 flip flops (24 in accumulator and 2 retimes)
--  There are 3355443 output clocks for every 16777216 input clocks.
--
-- Divider summary :
--
...
--
--  26    tbd       -6e-08    1e-08     use_phase_accumulator
--  29    tbd       -6e-08    5e-09     use_phase_accumulator improve_duty_cycle
--  6     tbd       0         0         use_recursive_controller
--  7     tbd       0         0         use_recursive_controller improve_duty_cycle
--  6     tbd       0         0         minimum_jitter
--  7     tbd       0         0         minimum_jitter improve_duty_cycle
--  6     tbd       0         0         (none)
--  7     tbd       0         0         (none) improve_duty_cycle
--
-- Warnings:
--* Warning: a fractional-N divider is not needed.
--*   Integer divider 5 is ok.
--
...
entity fracn_20 is
    ...
    port (
        ...
        output_50         : out std_logic;  -- 20000000 Hz output - approx 50% duty cycle
        output_pulse      : out std_logic   -- 20000000 Hz output - high for single clock per cycle
    );
end fracn_20;

architecture rtl of fracn_20 is

    -- definitions for prescaler / controller design
    constant n              : positive := 5;    -- prescaler divides by n or n + 1
    constant a              : positive := 1;    -- this many counts of 5
    constant b              : natural  := 0;    -- this many counts of 6
    ...
    constant c              : unsigned(num_bits - 1 downto 0) := to_unsigned(3355443, num_bits);
...
--------------------------------------------------------------------------------
-- Prescaler.  Divides by either 5 or 6
