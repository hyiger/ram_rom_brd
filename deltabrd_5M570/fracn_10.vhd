...
-- Command Line : ./fracn09.pl -e fracn_10 100e6 10e6
-- Date         : Fri Jul 26 19:21:53 2019
...
--  Approx 7 flip flops (4 in prescaler, 1 in controller and 2 retimes).
--  The recursive controller uses approx 1 flip flop.
--  The Dual-Modulus Prescaler uses ratios /10,/11
--  The Output consists of 1 cycle of 10 input clocks,
--  and 0 cycles of 11 input clocks.
--  There is 1 output clock for every 10 input clocks.
--
-- Design Parameters (use_phase_accumulator = TRUE) :
--  Approx 27 flip flops (25 in accumulator and 2 retimes)
--  There are 3355443 output clocks for every 33554432 input clocks.
--
-- Divider summary :
--
...
--
--  27    tbd       -6e-08    1e-08     use_phase_accumulator
--  30    tbd       -6e-08    5e-09     use_phase_accumulator improve_duty_cycle
--  7     tbd       0         0         use_recursive_controller
--  8     tbd       0         0         use_recursive_controller improve_duty_cycle
--  7     tbd       0         0         minimum_jitter
--  8     tbd       0         0         minimum_jitter improve_duty_cycle
--  7     tbd       0         0         (none)
--  8     tbd       0         0         (none) improve_duty_cycle
--
-- Warnings:
--* Warning: a fractional-N divider is not needed.
--*   Integer divider 10 is ok.
--
...
entity fracn_10 is
    ...
    port (
        ...
        output_50         : out std_logic;  -- 10000000 Hz output - approx 50% duty cycle
        output_pulse      : out std_logic   -- 10000000 Hz output - high for single clock per cycle
    );
end fracn_10;
...
architecture rtl of fracn_10 is
    ...
    constant n              : positive := 10;   -- prescaler divides by n or n + 1
    constant a              : positive := 1;    -- this many counts of 10
    constant b              : natural  := 0;    -- this many counts of 11
...
--------------------------------------------------------------------------------
-- Prescaler.  Divides by either 10 or 11
